...
`Prometheus`将监控数据按照`2h`分割成一个`block`。`Prometheus`将当前`block`的数据保存在内存中，然后会通定期将数据刷到磁盘。




为了防止应用崩溃而导致内存数据丢失，`Prometheus`实现了`WAL(write-ahead log)`机制。
随着时间增长，`block`


启动时配置`storage.tsdb.path`

```
./prometheus  --config.file=documentation/examples/prometheus.yml   --storage.tsdb.path=<your storage pzth>
```

每个block就是`storage.tsdb.path`下的一个子目录，每个block目录包含一下几部分：
- meta.json   元信息
- tombstones
- index
- chunks 
