...
- `name`   配置名称，用于标识，根据需求起名字
- `type`   项目的开发语言

- `program`  `go`项目的`main.go` 文件路径，即启动入口文件路径
- `args`   数组类型，项目启动时的参数
- `${workspaceFolder}`  是`vscode`的内置变量，表示项目根目录的路径



## 二开：记录`prometheus`拉取指标的请求信息
...
```
 ...
 `prometheus`是通过定期(*配置`scrape_interval`*)向监控目标发送`http`请求(*默认接口`GET /metrics`*)获取监控指标的。上述配置`prometheus`每`15s`向`http://localhost:9090/metrics`发送http请求获取监控指标，请求超时时间为10s。
