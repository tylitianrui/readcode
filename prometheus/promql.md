...
先看下面案例  
**`prometheus_http_requests_total{handler="/metrics"}`** 图形

![prometheus_http_requests_total_raw.png](./src/prometheus_http_requests_total_raw.png) 
<br>
**`rate(prometheus_http_requests_total{handler="/metrics"}[1m])`** 图形
...
(146-132) ➗ 60s ~= 0.233
```
`0.233` 不等于表里的 `0.289`。这是什么原因呢？这就是本节要说的**数据外推**。

<br>

`rate`和 `increase`函数都需要计算时间区间的第一个和最后一个样本，也就选取区间上的两个边界数据。现实情况，采集的样本并不是完全与采样间隔重合，总是会出现少量的时间偏移。
